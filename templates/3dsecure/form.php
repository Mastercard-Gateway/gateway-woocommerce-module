...
<?php if (!isset($authenticationRedirect['acsUrl']) || !isset($authenticationRedirect['paReq'])): ?>
    <body>
    <p>Data Error</p>
    </body>
<?php else: ?>
    <body onload="return window.document.echoForm.submit()">
    <form name="echoForm" method="post" action="<?php echo $authenticationRedirect['acsUrl'] ?>" accept-charset="UTF-8" id="echoForm">
        ...
        <noscript>
            <div id="msg">
                <div id="submitButton">
                    <input type="submit" value="<?php echo __( 'Click here to continue', 'woocommerce' ) ?>" class="button" />
                </div>
            </div>
        </noscript>
    </form>
